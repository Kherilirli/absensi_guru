...
const TeacherTable = () => {
  ...
  const fetchGuruData = useCallback(async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/presensi/role/guru', {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/users', newData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleEdit = (item) => {
    ...
    if (!item?.id) {
      console.error("ID guru tidak ditemukan saat akan diedit:", item);
      return;
    }
